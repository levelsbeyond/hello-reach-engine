...
class Search extends React.Component {
	...
	getThumbs() {
		...
		this.state.searchData.forEach(function(asset, index) {
			...
			if(asset.inventoryKey === 'collection' || asset.inventoryKey === 'audio' || asset.inventoryKey === 'project'){
				  // no thumbs for these.
					asset.thumbnail = `<img src="" height="36" width="36"/>`;
			} else {
				...
				// img tag for embed
				asset.thumbnail=`<img src="${this.props.authenticationPayload.reachEngineUrl}/${thubmnailUrl}" height="36" width="36"/>`;
			}
			results.push(asset);
		}, this);
